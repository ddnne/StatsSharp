...
namespace StatsSharp.Test.Probability.Distribution
{
    ...
    public class Multinomial
    {
        ...
        {
            var multi = new StatsSharp.Probability.Distribution.Discrete.Multinomial();
            var probs = new List<double>() { 0.3, 0.7 };

            var sample = new List<int>() { 1, 3 };
            var likelihood = multi.GetLikelihoodFunction(sample);
            var parameter = new StatsSharp.Probability.Parameter.Multinomial(probs, sample.Sum());
        ...
        public void TestMaxValueProbabilityDensityFunction()
        {
            ...
            var probs = new List<double>() { 0.3, 0.7 };
            var numOfTrials = 10;
            var parameter = new StatsSharp.Probability.Parameter.Multinomial(probs, numOfTrials);
